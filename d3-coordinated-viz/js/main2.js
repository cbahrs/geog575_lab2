    ...
    (function(){
        ...
        var attrArray = ["child0_18","adult19_25","adult26_34","adult35_54","adult55_64","elder65up"];
...
console.log("After setmap");
            ...
            var promises = [];
            promises.push(d3.csv("data/popByAge2.csv")); //load attribute data
            promises.push(d3.json("data/usStates2.topojson")); //load spatial data
...
//create coordinated bar chart
        function setChart(ageData, colorScale){
            ...
            var bars = chart.selectAll(".bar")
                ...
                .attr("width", chartInnerWidth / ageData.length - 1)
                .on("mouseover", function(d){
                    highlight(d);
                })
                .on("mouseout", function(d){
            ...
            updateChart(bars, ageData.length, colorScale);
        }; //end setChart()


//***CAN MOVE LOCATION IN style.css
//create dropdown for attribute selection
        function createDropdown(ageData){
            //add select element
            var dropdown = d3.select("body")
                .append("select")
                .attr("class", "dropdown")
                .on("change", function(){
                    changeAttribute(this.value, ageData)
                });
...
//dropdown change listener handler
        function changeAttribute(attribute, ageData){
            //change the expressed value
            expressed = attribute;

            //change yscale dynamically
            csvmax = d3.max(ageData, function(d) {
...
//function to create dynamic label
        function setLabel(props){
            //label content
            var labelAttribute = "<b>"+props.NAME+"</b><br><br><b>"+expressed+
            ...
            var infolabel = d3.select("body")
                ...
                .attr("id", props.NAME+"label") //for styling label
